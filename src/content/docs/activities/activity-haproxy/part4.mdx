...
curl -I [http://node2.example.test/](http://node2.example.test/)
```
...
#### 3.4.3. Si rien n’apparaît : checklist rapide
...
#### 3.4.4. Rendre les logs persistants (facultatif mais utile)
...
```shell
curl -I [http://172.25.](http://172.25.)x.2:8404/haproxy?stats
```
...
```bash
watch -n1 'curl -s [http://node1.example.test/](http://node1.example.test/) | grep "Server"'
```
