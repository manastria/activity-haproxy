...
```bash
for i in {1..10}; do curl -s [http://node1.example.test/](http://node1.example.test/) | grep -E "node1|node2"; done
```
...
1.  **Répartition** :
    ...
    ```bash
    for i in {1..8}; do curl -s [http://node1.example.test/](http://node1.example.test/) | grep -E "node1|node2"; done
    ```
